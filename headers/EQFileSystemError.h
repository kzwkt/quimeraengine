...
/// </summary>
class QE_LAYER_SYSTEM_SYMBOLS EQFileSystemError
{
    // ENUMERATIONS
    // ---------------
public:
    ...
    enum EnumType
    {
        E_Success = QE_ENUMERATION_MIN_VALUE, /*!< The operation completed successfully. */
        E_Unknown,          /*!< An error occurred which is not typified yet. */
        E_DoesNotExist,     /*!< The file or directory does not exist. */
        E_FileIsTooLarge,    /*!< The file is too large, more than what the file system can manage. */
        E_NameIsTooLong,    /*!< The name of the file or directory is too long. */
        E_NoSpaceInDevice,  /*!< There is not enough free space in the device.  */
        E_NoPermissions,    /*!< The current user does not have permissions to perform the operation on a given file or directory. */
        E_AlreadyExists,    /*!< The file or directory already exists. */
...
public:
    ...
    typedef std::map<string_q, EQFileSystemError::EnumType> TNameValueMap;
    typedef std::pair<string_q, EQFileSystemError::EnumType> TNameValuePair;
...
public:
    ...
    inline EQFileSystemError(const EQFileSystemError::EnumType eValue) : m_value(eValue)
    {
    }
    ...
    inline EQFileSystemError(const enum_int_q nValue) : m_value(scast_q(nValue, const EQFileSystemError::EnumType))
    {
    }
    ...
    inline explicit EQFileSystemError(const string_q &strValueName)
    {
        *this = strValueName;
    }
    ...
    inline EQFileSystemError(const EQFileSystemError &eValue) : m_value(eValue.m_value)
    {
    }
...
public:
    ...
    /// </returns>
    inline EQFileSystemError& operator=(const enum_int_q nValue)
    {
        m_value = scast_q(nValue, const EQFileSystemError::EnumType);
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQFileSystemError& operator=(const string_q &strValueName)
    {
        if(EQFileSystemError::sm_mapValueName.find(strValueName) != EQFileSystemError::sm_mapValueName.end())
            m_value = sm_mapValueName[strValueName];
        else
            m_value = EQFileSystemError::_NotEnumValue;
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQFileSystemError& operator=(const EQFileSystemError::EnumType eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQFileSystemError& operator=(const EQFileSystemError &eValue)
    {
    ...
    /// </returns>
    bool operator==(const EQFileSystemError &eValue) const
    {
    ...
    {
        return m_value == scast_q(nValue, const EQFileSystemError::EnumType);
    }
    ...
    /// </returns>
    bool operator==(const EQFileSystemError::EnumType eValue) const
    {
    ...
    static const std::vector<EnumType>& GetValues()
    {
        ...
        if(arValues.empty())
        {
            const size_t ENUM_ARRAY_COUNT = EQFileSystemError::sm_mapValueName.size();
            ...
            for(size_t i = 0; i < ENUM_ARRAY_COUNT; ++i)
                arValues.push_back(EQFileSystemError::sm_arValueName[i].second);
        }
    ...
    /// </returns>
    inline operator EQFileSystemError::EnumType() const
    {
        return m_value;
    }
    ...
    {
        return ConvertToString(m_value, EQFileSystemError::sm_mapValueName);
    }
...
private:
    ...
    // </returns>
    const string_q& ConvertToString(const EQFileSystemError::EnumType eValue, const TNameValueMap& nameValueDictionary) const
    {
...
private:
    ...
    /// The contained enumeration value.
    /// </summary>
    EQFileSystemError::EnumType m_value;
