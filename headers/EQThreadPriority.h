...
public:
    ...
    typedef std::map<string_q, EQThreadPriority::EnumType> TNameValueMap;
    typedef std::pair<string_q, EQThreadPriority::EnumType> TNameValuePair;
...
public:
    ...
    inline EQThreadPriority(const EQThreadPriority::EnumType eValue) : m_value(eValue)
    {
    }
    ...
    inline EQThreadPriority(const enum_int_q nValue) : m_value(scast_q(nValue, const EQThreadPriority::EnumType))
    {
    }
    ...
    inline explicit EQThreadPriority(const string_q &strValueName)
    {
        *this = strValueName;
    }
    ...
    inline EQThreadPriority(const EQThreadPriority &eValue) : m_value(eValue.m_value)
    {
    }
...
public:
    ...
    /// </returns>
    inline EQThreadPriority& operator=(const enum_int_q nValue)
    {
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQThreadPriority& operator=(const string_q &strValueName)
    {
        if(EQThreadPriority::sm_mapValueName.find(strValueName) != EQThreadPriority::sm_mapValueName.end())
            m_value = sm_mapValueName[strValueName];
        else
            m_value = EQThreadPriority::_NotEnumValue;
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQThreadPriority& operator=(const EQThreadPriority::EnumType eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQThreadPriority& operator=(const EQThreadPriority &eValue)
    {
    ...
    static const std::vector<EnumType>& GetValues()
    {
        ...
        if(arValues.empty())
        {
            const size_t ENUM_ARRAY_COUNT = EQThreadPriority::sm_mapValueName.size();
            ...
            for(size_t i = 0; i < ENUM_ARRAY_COUNT; ++i)
                arValues.push_back(EQThreadPriority::sm_arValueName[i].second);
        }
    ...
    /// </returns>
    inline operator EQThreadPriority::EnumType() const
    {
        return m_value;
    }
    ...
    {
        return ConvertToString(m_value, EQThreadPriority::sm_mapValueName);
    }
...
private:
    ...
    // </returns>
    const string_q& ConvertToString(const EQThreadPriority::EnumType eValue, const TNameValueMap& nameValueDictionary) const
    {
