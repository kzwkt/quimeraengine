...
public:
    ...
    typedef std::map<string_q, EQNormalizationForm::EnumType> TNameValueMap;
...
public:
    ...
    inline explicit EQNormalizationForm(const string_q &strValueName)
    {
        *this = strValueName;
    }
    ...
    inline EQNormalizationForm(const EQNormalizationForm &eValue) : m_value(eValue.m_value)
    {
    }
...
public:
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQNormalizationForm& operator=(const string_q &strValueName)
    {
        if(EQNormalizationForm::sm_mapValueName.find(strValueName) != EQNormalizationForm::sm_mapValueName.end())
            m_value = sm_mapValueName[strValueName];
        else
            m_value = EQNormalizationForm::_NotEnumValue;
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQNormalizationForm& operator=(const EQNormalizationForm &eValue)
    {
    ...
    static const std::vector<EnumType>& GetValues()
    {
        ...
        if(arValues.empty())
        {
            const size_t ENUM_ARRAY_COUNT = EQNormalizationForm::sm_mapValueName.size();
            ...
            for(size_t i = 0; i < ENUM_ARRAY_COUNT; ++i)
                arValues.push_back(EQNormalizationForm::sm_arValueName[i].second);
        }
    ...
    /// </returns>
    inline operator EQNormalizationForm::EnumType() const
    {
        return m_value;
    }
    ...
    {
        return ConvertToString(m_value, EQNormalizationForm::sm_mapValueName);
    }
