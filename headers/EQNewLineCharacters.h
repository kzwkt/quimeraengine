...
public:
    ...
    EQNewLineCharacters(const EQNewLineCharacters::EnumType eValue) : m_value(eValue)
    {
    }
    ...
    EQNewLineCharacters(const enum_int_q nValue) : m_value(scast_q(nValue, const EQNewLineCharacters::EnumType))
    {
    }
    ...
    EQNewLineCharacters(const char* szValueName)
    {
        *this = szValueName;
    }
    ...
    EQNewLineCharacters(const EQNewLineCharacters &eValue) : m_value(eValue.m_value)
    {
    }
    ...
    /// </returns>
    EQNewLineCharacters& operator=(const enum_int_q nValue)
    {
    ...
    /// The enumerated type itself.
    /// </returns>
    EQNewLineCharacters& operator=(const char* szValueName)
    {
        bool bMatchFound = false;
        unsigned int uEnumStringIndex = 0;

        while(!bMatchFound && uEnumStringIndex < EQNewLineCharacters::_GetNumberOfValues())
        {
            bMatchFound = strcmp(sm_arStrings[uEnumStringIndex], szValueName) == 0;
            ++uEnumStringIndex;
        }

        QE_ASSERT_ERROR(uEnumStringIndex < EQNewLineCharacters::_GetNumberOfValues(), "The input string does not correspond to any valid enumeration value.");
    ...
    /// The enumerated type itself.
    /// </returns>
    EQNewLineCharacters& operator=(const EQNewLineCharacters::EnumType eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    EQNewLineCharacters& operator=(const EQNewLineCharacters &eValue)
    {
    ...
    static const Kinesis::QuimeraEngine::Common::DataTypes::QArrayBasic<const EnumType> GetValues()
    {
        ...
        static const QArrayBasic<const EnumType> ARRAY_OF_VALUES(sm_arValues, EQNewLineCharacters::_GetNumberOfValues());
    ...
    /// </returns>
    operator EQNewLineCharacters::EnumType() const
    {
        return m_value;
    }
...
private:
    ...
    /// </returns>
    inline static const char* _ConvertToString(const EQNewLineCharacters::EnumType eValue)
    {
        QE_ASSERT_ERROR(scast_q(eValue, unsigned int) < EQNewLineCharacters::_GetNumberOfValues(), "The enumeration value is not valid.");
...
private:
    ...
    /// </summary>
    static const EQNewLineCharacters::EnumType sm_arValues[];
