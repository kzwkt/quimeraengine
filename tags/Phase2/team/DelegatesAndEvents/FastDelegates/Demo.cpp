...
public:
    ...
    virtual void SimpleVirtualFunction(int num, char *str) {
		printf("In YEAHEYAHEYAHEYAH in %s. Num=%d, str = %s\n", m_name, num, str);	}
};
...
int main(void)
{
	...
	// You can also bind directly using the constructor
        MyDelegate dg = MyDelegate(&d, &COtherClass::SimpleVirtualFunction);
        dg(1, 0);
        FastDelegate1<int>::Params yeah;
        yeah.p = 0;
        //MyDelegate::GenericMemFn xx = &CBaseClass::SimpleVirtualFunction;
