...
#ifndef __EQITERATIONDIRECTION__
#define __EQITERATIONDIRECTION__
...
/// </summary>
class QE_LAYER_TOOLS_SYMBOLS EQIterationDirection
{
    // ENUMERATIONS
    // ---------------
public:
    ...
    enum EnumType
    {
        E_Forward = QE_ENUMERATION_MIN_VALUE, /*!< Forward iteration, from the first element to the last one. */
        E_Backward,                           /*!< Backward iteration, from the last element to the first one. */
...
public:
    ...
    typedef std::map<string_q, EQIterationDirection::EnumType> TNameValueMap;
...
public:
    ...
    inline EQIterationDirection(const EQIterationDirection::EnumType &eValue) : m_value(eValue)
    {
    }
    ...
    inline EQIterationDirection(const enum_int_q &nValue) : m_value(scast_q(nValue, const EQIterationDirection::EnumType))
    {
    }
    ...
    inline explicit EQIterationDirection(const string_q &strValueName)
    {
        *this = strValueName;
    }
    ...
    inline EQIterationDirection(const EQIterationDirection &eValue) : m_value(eValue.m_value)
    {
    }
...
public:
    ...
    /// </returns>
    inline EQIterationDirection& operator=(const enum_int_q &nValue)
    {
        m_value = scast_q(nValue, const EQIterationDirection::EnumType);
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQIterationDirection& operator=(const string_q &strValueName)
    {
        if(EQIterationDirection::sm_mapValueName.find(strValueName) != EQIterationDirection::sm_mapValueName.end())
            m_value = sm_mapValueName[strValueName];
        else
            m_value = EQIterationDirection::_NotEnumValue;
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQIterationDirection& operator=(const EQIterationDirection::EnumType &eValue)
    {
        m_value = eValue;
        return *this;
    }
    ...
    /// The enumerated type itself.
    /// </returns>
    inline EQIterationDirection& operator=(const EQIterationDirection &eValue)
    {
    ...
    /// </returns>
    bool operator==(const EQIterationDirection &eValue) const
    {
    ...
    {
        return m_value == scast_q(nValue, const EQIterationDirection::EnumType);
    }
    ...
    /// </returns>
    bool operator==(const EQIterationDirection::EnumType &eValue) const
    {
    ...
    static const std::vector<EnumType>& GetValues()
    {
        ...
        if(arValues.empty())
        {
            const size_t ENUM_ARRAY_COUNT = EQIterationDirection::sm_mapValueName.size();
            ...
            for(size_t i = 0; i < ENUM_ARRAY_COUNT; ++i)
                arValues.push_back(EQIterationDirection::sm_arValueName[i].second);
        }
    ...
    /// </returns>
    inline operator EQIterationDirection::EnumType() const
    {
        return m_value;
    }
    ...
    {
        return ConvertToString(m_value, EQIterationDirection::sm_mapValueName);
    }
...
private:
    ...
    // </returns>
    const string_q& ConvertToString(const EQIterationDirection::EnumType& eValue, const TNameValueMap& nameValueDictionary) const
    {
...
private:
    ...
    /// The contained enumeration value.
    /// </summary>
    EQIterationDirection::EnumType m_value;
...
#endif // __EQITERATIONDIRECTION__
