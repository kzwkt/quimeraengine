...
static const QNTree<char>* GetSampleTree()
{
    ...
    if(!bInitialized)
    {
        ...
        QNTree<char>::QNTreeIterator iteratorA = sampleTree.GetRoot(EQTreeTraversalOrder::E_DepthFirstPreOrder);
...
static void BuildSampleTree(QNTree<char> &inputTree)
{
    ...
    QNTree<char>::QNTreeIterator iteratorA = inputTree.GetRoot(EQTreeTraversalOrder::E_DepthFirstPreOrder);
...
QTEST_CASE ( OperatorDereferencing_ReturnsTheCorrespondingElement_Test )
{
    ...
    QNTree<TestStructure>::QNTreeIterator iteratorA = SAMPLE_TREE.GetRoot(EQTreeTraversalOrder::E_DepthFirstPreOrder);
