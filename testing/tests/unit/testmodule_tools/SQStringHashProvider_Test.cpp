...
QTEST_SUITE_BEGIN( SQStringHashProvider_TestSuite )

/// <summary>
/// Checks that it returns zero when the input value is empty.
/// </summary>
QTEST_CASE ( GenerateHashKey_ItReturnsZeroWhenInputIsEmpty_Test )
{
    // [Preparation]
    const string_q INPUT_VALUE("");
    const pointer_uint_q NUMBER_OF_BUCKETS = 4;
    const pointer_uint_q EXPECTED_VALUE = 0;

    // [Execution]
    pointer_uint_q uHashKey = SQStringHashProvider::GenerateHashKey(INPUT_VALUE, NUMBER_OF_BUCKETS);
...
{
    // [Preparation]
    const string_q INPUT_VALUE("A text to be used to generate a hash key");
    const pointer_uint_q NUMBER_OF_BUCKETS = 500;
    const pointer_uint_q EXPECTED_VALUE = 229;
...
// End - Test Suite: SQStringHashProvider
QTEST_SUITE_END()
