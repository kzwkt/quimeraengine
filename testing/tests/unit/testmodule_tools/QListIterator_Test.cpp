...
{/* [TODO] Thund: Uncomment when the method Add and Remove exist
    // [Preparation]
    QList<int> SOURCE_LIST(10);
    for(unsigned int i = 0; i < SOURCE_LIST.GetCapacity(); ++i)
    {
...
QTEST_CASE ( OperatorEquality_ReturnsTrueWhenIteratorsAreEqual_Test )
{
    ...
    QList<int>::QListIterator ITERATOR_A(&LIST_A, 0);
    QList<int>::QListIterator ITERATOR_B(&LIST_A, 0);
...
{/* [TODO] Thund: Uncomment when the methods Add and Remove exist
    ...
    QList<int>::QListIterator ITERATOR_FIRST(SOURCE_LIST);
    QList<int>::QListIterator INVALID_ITERATOR(SOURCE_LIST);
    INVALID_ITERATOR.MoveLast();
    SOURCE_LIST.Remove(SOURCE_LIST.GetCount()-1);
